...
void CodeEditor::onSelectionChanged()
{
	...
	if (selected.isEmpty())
	{
		...
		for (auto blockIndex = 0; blockIndex < doc->blockCount(); ++blockIndex)
		{
			...
			for (auto& format : block.textFormats())
			{
				if (format.format.background() == QBrush(QColor(0, 125, 255, 125)))
				{
...
void CodeEditor::handleSelectionQuery(QTextCursor cursor)
{
	...
	QTextCharFormat fmt;
	fmt.setBackground(QBrush(QColor(0, 125, 255, 125)));
	searchIterator.setCharFormat(fmt);
